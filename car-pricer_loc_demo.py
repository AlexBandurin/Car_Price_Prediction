...
listing_columns = ['Year','Make','Model','Odometer','Condition','Color','Cylinders','Transmission','Fuel','Drive','Title',\
                   'City','Price']
...
#     site_map = folium.Map(location= [38.024612, -102.044790], zoom_start=5.4)
...
app.layout = html.Div(children=[
                                ...
                                html.Br(),
                                html.Br(),
                                html.H1(['This application uses a machine learning algorithm trained on tens of thousands of used car listings that have \
                                        been obtained through web scraping of classifieds websites. The model is created using XGBoost for regression as well as BERT for NLP.',\
                                        html.Br(),html.Br(),\
                                        'Give it a go and let me know what you think!',html.Br()],
                                        style={'font-family': 'Arial', 'margin-left': '0%','margin-right': '50%','font-size': 20, 'color': '#4d423d' }),
                                html.H1([
                                        "You can find the code and more details about this project ",
                                        html.A('here', href='https://github.com/AlexBandurin/car_price_prediction/tree/master', target='_blank'),
                                        "."],
                                        style={'font-family': 'Arial', 'margin-left': '0%','margin-right': '50%','font-size': 20, 'color': '#4d423d' }
                                ),
                                html.Div(
                                    children=[
                                        ...
                                        html.H1(['Vehicle listings with the same Make and Model and Year (± 2) as the one \
                                                you selected (retrieved from my Azure SQL Database):',\
                                                 html.Br(), 'This data is also shown in the interactive map below. Click on each marker for more info on that vehicle.'
                                                ], style={'textAlign': 'left','font-size': 28, 'color': '#4a4141'}),  
                                        html.Div(
                                            children=[
                                                html.Iframe(id='map', srcDoc= '', width='100%', height='715')
                                            ],
                                            style={
                                                'display': 'flex',
...
def get_price(clicks, year, make, model, odometer, cylinders, condition, color, title, fuel, transmission, drive, description):
        ...
        and all([isinstance(var, int) and var for var in [year, odometer]]):
            print("Button pressed, input is good")
            try:
                # Define Folium Map
                def map(df):
                    print('map start')
                    ...
                    for i in range(length):
                        print('start of map for loop')
                        cooridinates = df.loc[i,["Lat", "Long"]]
                        ...
                        marker = folium.map.Marker(
                            ...
                            icon=DivIcon(
                                ...
                                popup= folium.Popup('<div style="font-size: 30px;"><b>Year:</b> '+str(df["Year"][i])+'<br>'\
                                                    +'<b>Make:</b> '+str(df["Make"][i])+'<br>'+'<b>Model:</b> '+str(df["Model"][i])\
                                                    +'<br><br>'+'<b>Color:</b> '+str(df["Color"][i])+'<br>'+'<b>Condition:</b> '+str(df["Condition"][i])\
                                                    +'<br>'+'<b>Mileage:</b> '+ "{:,}".format(df["Odometer"][i])+'<br><br>' +'<b>Title:</b> '+str(df["Title"][i])+'<br>'\
                                                    +'<b>Cylinders:</b> '+str(df["Cylinders"][i])+'<br>' +'<b>Transmission: </b>'+str(df["Transmission"][i])\
                                                    +'<br><br>'+'<span style="color:green;"><b>Price:</b> '+ str(df["Price"][i])+'</span></div>',\
                ...
                query = f"SELECT * FROM car_data_loc WHERE Make = '{make}' and Model = '{model}' and Year BETWEEN {year - 1} AND {year + 1} ORDER BY Year, Odometer"
                ...
                df_map = pd.DataFrame(dict_result)
                ...
                if response.status_code == 200:
                    print('Good response')
                    price = response.text
                    map_html = map(df_map)  # Generate the map and get its HTML content
                    print('last ckeckpoint')

                    return ("Your Vehicle Price is:    $" + str('{:,}'.format(int(price)))), dict_result, map_html
                else:
